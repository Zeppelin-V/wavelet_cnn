...
class wavelet_transform():
    ...
    def transform(self, data):
        ...
        return torch.stack((torch.unsqueeze(torch.Tensor(coeffs[0]).to(self.computing_device), 2),
                            torch.unsqueeze(torch.Tensor(coeffs[1]).to(self.computing_device), 2),
                            torch.unsqueeze(torch.Tensor(coeffs[2]).to(self.computing_device), 2),
                            torch.unsqueeze(torch.Tensor(coeffs[3]).to(self.computing_device), 2)), dim=2)
